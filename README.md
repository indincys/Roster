...
本仓库实现 `PLAN.md` 中的跨平台 Electron 桌面应用。v1 采用本地优先、多品牌工作空间隔离、SQLite 数据库和类型化 IPC。
...
Skill 中心用于安装、编辑、快照和复原 Skill；标题工作区和文案工作区通过 typed preload IPC 流式生成并支持取消。图片工作室第一步通过已启用的 `image_prompt` Skill 和文本 LLM 生成提示词，图片生成步骤再调用图片 Provider。当前开发验证可使用 mock Provider；真实 Provider 需要在设置页保存对应 API key 后测试连接。
...
10. 在保存真实 API key 且网络可用的环境里验证 OpenAI/Anthropic/Gemini/OpenAI Image 的成功调用，确认标题/文案/图片输出和脱敏日志。
